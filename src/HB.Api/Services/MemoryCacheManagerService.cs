...
namespace HB.Api.Services
{
    ...
    public class MemoryCacheManagerService : ICacheManagerService
    {
        private readonly IEasyCachingProvider _provider;

        private readonly HBConfiguration _hBConfiguration;

        public MemoryCacheManagerService(IEasyCachingProvider provider, HBConfiguration hBConfiguration)
        {
            _provider = provider;
        ...
        public T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            return _provider.Get<T>(key, acquire, TimeSpan.FromMinutes(cacheTime ?? _hBConfiguration.CacheTime))
                .Value;
        }
        ...
        public async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            ...
            var t = await _provider.GetAsync(key, acquire, TimeSpan.FromMinutes(cacheTime ?? _hBConfiguration.CacheTime));
            return t.Value;
        }
        ...
        public void Set(string key, object data, int? cacheTime = null)
        {
            ...
            _provider.Set(key, data, TimeSpan.FromMinutes(cacheTime ?? _hBConfiguration.CacheTime));
        }
