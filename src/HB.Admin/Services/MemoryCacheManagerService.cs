...
namespace HB.Admin.Services
{
    public class MemoryCacheManagerService : ICacheManagerService
    {
        ...
        public MemoryCacheManagerService(IEasyCachingProvider provider)
        {
            _provider = provider;
        }
        ...
        public T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            return _provider.Get<T>(key, acquire, TimeSpan.FromMinutes(cacheTime ?? HBCachingDefaults.CacheTime))
                .Value;
        }
        ...
        public async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            ...
            var t = await _provider.GetAsync(key, acquire, TimeSpan.FromMinutes(cacheTime ?? HBCachingDefaults.CacheTime));
            return t.Value;
        }
        ...
        public void Set(string key, object data, int? cacheTime = null)
        {
            ...
            _provider.Set(key, data, TimeSpan.FromMinutes(cacheTime ?? HBCachingDefaults.CacheTime));
        }
